...
export default function Home() {
  ...
  const SectionWrapper = ({ title, children, className = "" }) => (
    <section
      className={
        "card bg-base-200 shadow-xl border border-base-300 w-full p-6 sm:p-8 space-y-6 rounded-2xl flex flex-col " +
        className
      }
    >
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-2 sm:px-6 lg:px-8 py-10 space-y-12 bg-base-100 text-base-content min-h-[calc(100vh-80px)] flex flex-col">
      {error && (
      ...
      <SectionWrapper title="Suggested Users">
        <div className="overflow-x-auto pb-2">
          <div className="flex space-x-6 min-w-max">
            {loading
              ? [...Array(6)].map((_, i) => (
                  <div key={i} className="overflow-hidden rounded-xl h-56 w-64 flex">
              ...
              : suggestedUsers.map((user) => (
                  <UserCard
                    ...
                    className="card bg-base-100 border border-base-300 shadow-md hover:shadow-lg p-4 rounded-xl overflow-hidden flex flex-col w-64"
                  />
                ))}
          </div>
        </div>
