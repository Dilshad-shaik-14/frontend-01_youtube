...
const VideoGrid = ({ title, videos, onSelect }) => (
  <section>
...
const WatchHistory = ({ history, deleting, onDelete, onSelect }) => (
  <section>
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Debugging logs
        console.log("Channel Avatar URL:", channelRes.data?.avatar);
        console.log("Channel Cover URL:", channelRes.data?.coverImage);
      } catch (err) {
        toast.error(
  ...
  return (
    <div className="min-h-screen bg-base-200 dark:bg-base-300 text-base-content px-10 py-14 max-w-[1440px] mx-auto space-y-12">
      ...
      <VideoGrid title="My Uploaded Videos" videos={channelVideos} onSelect={setSelectedVideo} />
