...
export default function VideoPlayerModal({ video, onClose }) {
  ...
  {video && (
    <motion.div
      className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md flex justify-center items-center px-4 py-6 overflow-y-auto"
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      exit={{ opacity: 0, scale: 0.95 }}
      transition={{ duration: 0.3, ease: "easeInOut" }}
    >
    <div className="relative w-full max-w-[90rem] max-h-[95vh] bg-zinc-900 text-white rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-50 bg-zinc-900/80 hover:bg-zinc-800 text-white p-2 rounded-full"
        >
          <X size={20} />
        </button>
        ...
        <div className="w-full md:w-2/3 aspect-video md:aspect-[16/9] flex flex-col max-h-[80vh]">
          ...
          <div className="p-4 space-y-3 border-t border-zinc-800 overflow-auto">
        ...
        <div className="w-full md:w-1/3 bg-zinc-950 border-l border-zinc-800 p-4 flex flex-col max-h-[95vh]">
          <textarea
            rows={2}
          ...
          <div className="mt-4 overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-blue-700 hover:scrollbar-thumb-blue-600 scrollbar-track-transparent pr-1">
            {comments.length === 0 ? (
